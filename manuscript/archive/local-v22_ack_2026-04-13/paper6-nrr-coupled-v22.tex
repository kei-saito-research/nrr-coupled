...
\textbf{Implementation}: A reference implementation accompanying this work is available at \url{https://github.com/kei-saito-research/nrr-coupled}.
\end{abstract}

\noindent\textbf{Keywords:} NRR-Coupled, dependency consistency, repair cost, falsifiable simulation, robustness

\section{Introduction}
This paper is one module in an ongoing Non-Resolution Reasoning (NRR) research program, not a closed endpoint. Companion papers cover foundational theory, text-to-state mapping, implementation behavior, and cross-domain transfer with distinct scopes\cite{saito2025nrr,saito2026phi,saito2026transfer}.
...
NRR is a derivation-and-evaluation framework, not a closed recipe; cp is one structure-sensitive instantiation evaluated under fixed simulation contracts with condition-bounded reporting.

\textbf{Boundary.}
Transfer: independent-candidate condition.
cp: dependent-candidate condition.
In this paper, uncoupled baseline means the same update pipeline with coupling disabled (\(\beta=0\)), aligned with the Transfer-style setting\cite{saito2026transfer}.

\subsection{Series Alignment: Notation and Metrics}
Notation contract: cp keeps the LLM interface fixed at \((\sigma/\delta,\text{target})\) and changes only deterministic client-side propagation, aligned with Transfer-style execution.
Metric contract: cp evaluates dependency-consistency violation rate and additional repair-operator count under identical operator streams for uncoupled/coupled comparison.
Boundary statement: claims are condition-bounded to declared dependency structures (D-dependent, D-independent, D-mismatch), not universal across unknown structures.
...
\end{enumerate}
The coupled rule in this paper is an evaluated instantiation, not an exhaustive policy family; alternative coupling policies can be tested under the same operator-stream parity protocol.
...
\[
w_{t+1}^{(k_t)}=\tilde{w}^{(k_t)},\qquad
w_{t+1}^{(i)}=\frac{\tilde{w}^{(i)}}{\sum_{j\neq k_t}\tilde{w}^{(j)}}\left(1-\tilde{w}^{(k_t)}\right)\ \ (i\neq k_t).
\]
...
\begin{table}[t]
\centering
\caption{Aggregate pairwise deltas at \(\beta=0.3\) across 45 samples per condition.}
...
For D-mismatch, \(\Delta R\) includes a small number of negative samples (min \(=-2\)), but the aggregate effect remains degradation (mean \(=+3.3111\)) because high-penalty streams dominate under the fixed repair-cap setting.
Table~\ref{tab:main_results} reports the aggregate \(\beta=0.3\) results. Figure~\ref{fig:vdiff} visualizes the condition-dependent shift in violation deltas, and Figure~\ref{fig:rdiff} shows the corresponding repair-cost deltas.
...
Under the later non-evaluative organizing lens used elsewhere in the series, cp is
best read as a supporting implementation-side result. Once dependency structure is
externally declared and validated, local propagation can sometimes preserve broader
state coherence without repeated full comparative reevaluation at every turn. The
present paper does not test that organizing principle directly; it shows only that
this propagation rule helps under directionally correct declared structure and hurts
under sign-mis-specified structure.
...
Read through that later organizing lens, the current result is best treated as a
bounded supporting example of declared-structure local propagation, not as a direct
test that evaluation universally disappears in deployment.
...
models---including Claude (Anthropic), ChatGPT and Codex (OpenAI), and Gemini (Google)---for their assistance in linguistic refinement,
...
Repository preprint snapshot. \url{https://github.com/kei-saito-research/nrr-transfer} (accessed 2026-03-05).
\end{thebibliography}
