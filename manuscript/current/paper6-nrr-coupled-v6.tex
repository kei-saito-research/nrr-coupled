...
\title{\textbf{NRR-Coupled (cp): Dependency-Consistency Evaluation Without Tautological References}}
...
\begin{abstract}
cp v3 used a coupled-generated reference trajectory and was vulnerable to circular evaluation. This version removes that dependence and evaluates coupled propagation by dependency consistency itself. We keep the same fixed interface (LLM emits \((\sigma/\delta,\text{target})\); client executes updates) and compare uncoupled baseline (\(\beta=0\)) with coupled variants (\(\beta\in\{0.1,0.3,0.5\}\)). Evaluation uses 15 seed-fixed random operator streams for each of 3 patterns (\(n=4,5,6\)); each stream targets only primary candidates for 12 turns. Main metrics are (i) turn-wise dependency-consistency violation rate and (ii) additional repair operators needed after the primary stream. At \(\beta=0.3\), coupled updates reduce violation rate in D-dependent by 0.2123 and reduce repair operations by 0.58 on average, while D-mismatch increases violation rate by 0.2302 and repair operations by 3.31. D-independent remains exactly equivalent under strict numerical tolerance \(10^{-12}\), and its violation metric is structurally uninformative because \(A_{\text{eval}}=0\) yields zero evaluation opportunities. These results support a conditional claim: cp improves consistency only when dependency structure is correct and degrades under sign-mismatched dependencies.
\end{abstract}
...
\textbf{Protocol motivation.}
To avoid circular scoring, this protocol does not use coupled-generated reference states as evaluation targets. Instead, it measures whether produced transitions satisfy declared dependency relations.
...
From `cp\_v5\_report.json` at \(\beta=0.3\): all criteria pass.
D-dependent improves on both metrics; D-mismatch degrades on both metrics; D-independent remains exactly equivalent.

\section{Discussion}
This protocol resolves the circular-reference problem by scoring consistency directly from declared dependencies, not from coupled-generated target states. Under this design, cp remains conditionally useful and clearly falsifiable: wrong dependency signs produce measurable degradation.
...
Artifacts:
\begin{itemize}
  \item `spec/nrr-coupled\_spec\_v6.md`
  \item `repro/coupled\_state\_sim.py`
  \item `repro/README\_v6.md`
  \item `repro/results/cp\_v5\_*.csv`
  \item `repro/results/cp\_v5\_report.json`
\end{itemize}
