...
Section~\ref{sec:update_rule} defines the coupled update and complexity bounds. Section~\ref{sec:evaluation_design} describes simulation settings and dependency conditions. Section~\ref{sec:metrics} defines evaluation metrics, and Section~\ref{sec:falsifiable_criteria} states pass/fail criteria. Section~\ref{sec:results} reports quantitative outcomes, followed by interpretation in Section~\ref{sec:discussion}, boundary conditions in Section~\ref{sec:limitations}, conclusion in Section~\ref{sec:conclusion}, and reproducibility details in Section~\ref{sec:reproducibility}.
...
\label{sec:falsifiable_criteria}
\begin{enumerate}
...
Artifacts:
\begin{itemize}
  \item specification document in `spec/`
  \item simulation script in `repro/`
  \item reproducibility guide in `repro/`
...
models---including Claude (Anthropic), ChatGPT and Codex (OpenAI)---for assistance in linguistic refinement,
